...
@section('title', 'Edit TP Modul - CASLAB')
...
@section('content')
    <div class="py-8">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            {{-- Header --}}
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-900">Edit TP Modul</h1>
                <p class="mt-2 text-gray-600">Perbarui detail tp modul Anda</p>
            </div>

            {{-- Form Card --}}
            <div class="bg-white rounded-xl shadow-lg p-8">
                <form action="{{ route('tpmoduls.update', $tpmodul) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        ...
                        <input id="judul" name="judul" type="text" required
                            value="{{ old('judul', $tpmodul->judul) }}" placeholder="Masukkan judul tp modul"
                    ...
                    <div class="mb-6">
                        ...
                        <input id="subjudul" name="subjudul" type="text" required
                            value="{{ old('subjudul', $tpmodul->subjudul) }}" placeholder="Masukkan subjudul tp modul"
                            class="appearance-none block w-full px-4 py-3 border @error('subjudul') border-red-300 @else border-gray-300 @enderror rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 transition">
                        @error('subjudul')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <select id="category_id" name="category_id" required
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}"
                                    {{ old('category_id', $tpmodul->category_id) == $category->id ? 'selected' : '' }}>
                                    {{ $category->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="12" required placeholder="Tulis deskripsi tp modul Anda di sini..."
                            class="appearance-none block w-full px-4 py-3 border @error('deskripsi') border-red-300 @else border-gray-300 @enderror rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 transition">{{ old('deskripsi', $tpmodul->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    <el-dialog>
        <dialog id="delete-dialog" aria-labelledby="delete-dialog-judul"
                ...
                <el-dialog-panel
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                <h3 id="delete-dialog-judul" class="text-base font-semibold text-gray-900">Hapus TP Modul
                                </h3>
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">Apakah Anda yakin ingin menghapus
                                        "{{ $tpmodul->judul }}"? Semua data tp modul akan dihapus secara permanen. Tindakan
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                        <form action="{{ route('tpmoduls.destroy', $tpmodul) }}" method="POST" class="inline">
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white hover:bg-red-500 sm:ml-3 sm:w-auto">Hapus</button>
                        </form>
